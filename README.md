...
| 탭 | 정적 동작 방식 | 비고 |
|---|---|---|
| 재무 분석 | 6개 기업 재무·AI의견 스냅샷 | 완전 동작 |
| 산업 인텔리전스 | 5개 섹터 + 6개 기업 산업동향 스냅샷 | 준비된 항목만, 국내 중심 |
| Macro Deal | 매크로 지표·분석·뉴스브리프 스냅샷 | 고정 시점 스냅샷(지표는 ECOS/FRED 키 없으면 예시값) |
| 인사이트 로그 | 브라우저 localStorage CRUD | 완전 동작, 브라우저별 저장·내보내기(MD/CSV/JSON) |

준비된 산업: 반도체 · 2차전지 · AI · 바이오텍 · 자동차
준비된 기업: 삼성전자 · SK하이닉스 · 카카오 · NAVER · 현대자동차 · LG전자

남은 비활성 기능(요청 시 "준비 중" 응답): 실시간 Deal Signal/Radar/Memo, 기사 단위 Deal Memo 등 매 요청마다 LLM이 필요한 것들. 이들도 대상을 정해 스냅샷으로 미리 구우면 정적으로 지원할 수 있다 — 강의 64~67회차의 확장 지점이다.
